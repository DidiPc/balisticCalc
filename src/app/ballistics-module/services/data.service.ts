    ...
    return combineLatest([
      ...
      map(([weather, target, firearms, firearmId, roundId]) => {
        ...
        if (weather && target && firearm && round) {
          ...
          let currentCrossWindDriftInches: number, currentDropInches: number, currentEnergyFtLbs: number, currentLeadInches: number,
            currentRangeMeters: number, currentRangeYards: number, currentTimeSeconds: number, currentVelocityFPS: number,
